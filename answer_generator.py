...
document_store = FAISSDocumentStore.load(index_path="data/my_index.faiss", config_path="data/my_config.json")
...
prompt_node = PromptNode("gpt-3.5-turbo", api_key="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", default_prompt_template=lfqa_prompt,model_kwargs={"stream":True})
...
def question_answering_bot(input_question):
    answer = query_pipeline.run(query=input_question, params={"Retriever": {"top_k": 2}})
...
# question_answering_bot("What are the different billing mehtods in SD ??")
